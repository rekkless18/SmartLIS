...
## BE-018: 部署指南环境变量真实化配置 ✅

**任务描述**：将DEPLOY_GUIDE.md中的环境变量配置从占位符替换为真实的示例值

**完成内容**：
1. **Supabase配置真实化**：
   - 将Supabase URL替换为真实格式：`https://xyzabcdefghijklmnop.supabase.co`
   - 提供完整的JWT格式的ANON_KEY和SERVICE_ROLE_KEY示例
   - 使用真实的Supabase项目引用格式

2. **JWT配置优化**：
   - 将JWT_SECRET替换为安全的长密钥：`smartlis_jwt_secret_key_2025_production_environment_secure_token`
   - 保持JWT_EXPIRES_IN为7天的合理配置

3. **服务器配置更新**：
   - 将API地址从占位符改为具体IP示例：`http://192.168.1.100:3000/api`
   - 保持端口配置为3000（后端）和5000（前端）
   - 维持应用名称和版本号的一致性

4. **配置格式标准化**：
   - 所有配置值都使用生产环境可用的真实格式
   - 移除所有"your_xxx"类型的占位符
   - 提供可直接复制使用的配置示例

**技术要点**：
- Supabase URL和密钥的标准JWT格式
- 生产环境安全密钥的长度和复杂度要求
- 内网IP地址的配置示例
- 环境变量的命名规范和格式统一
...
# JWT配置（安全密钥）
JWT_SECRET=smartlis_jwt_secret_key_2025_production_environment_secure_token
...
**任务描述**：修复部署后登录时出现401 Unauthorized错误的问题

**问题分析**：
- 前端API配置指向localhost:3000，但实际访问远程服务器47.106.198.192:5000
- 后端服务配置端口为3000，与用户访问端口5000不匹配
- 端口配置不一致导致API请求失败

**解决方案**：
1. **后端端口配置修复**：
   - 修改backend/.env文件，将PORT从3000改为5000
   - 确保后端服务监听正确的端口

2. **前端API配置修复**：
   - 修改frontend/.env文件，将VITE_API_BASE_URL端口从3000改为5000
   - 统一前后端端口配置

3. **端口冲突解决**：
   - 使用netstat命令检查端口占用情况
   - 终止占用5000端口的进程（PID: 39872, 12720）
   - 重启后端服务，确保在5000端口正常运行
...
# 终止占用进程
taskkill /F /PID 39872
taskkill /F /PID 12720
...
```bash
# backend/.env
PORT=5000  # 从3000改为5000

# frontend/.env
VITE_API_BASE_URL=http://47.106.198.192:5000/api  # 端口从3000改为5000
```
